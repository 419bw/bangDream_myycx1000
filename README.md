...
两套模型共用相同的基础流程：先根据当前活动 ID 获取活动元信息和当前榜线数据，再扫描历史活动，筛选同类型或可用的大盘历史数据作为训练样本。每次预测会从当前已知分数点开始，按固定时间步长向活动结束时间递推，最终取最后一个预测点作为最终 PT。

### LightGBM 版本

LightGBM 版本采用分阶段建模。程序会把历史活动按时间位置切成 `start`、`middle`、`penultimate`、`end` 四个阶段，分别训练对应模型，再在预测时根据当前模拟时间切换使用不同阶段的模型。

核心特征包括当前分速、滞后窗口、活动已进行时间、距离结束时间、是否周末、一天内时间进度、24 小时前分速参考以及活动强度等。模型使用非对称损失函数，对低估结果施加更高惩罚，使预测更倾向于避免低估，整体结果可能偏高。预测阶段使用递归接力方式：模型先预测下一个时间点的增长速度，再把这个预测点加入缓冲区，继续预测后续时间点。

训练完成后，模型、特征列表和归一化器会缓存在 `models_overlap/` 中。配置变化会改变缓存哈希，从而触发重新训练。

### LSTM 版本

LSTM 版本把活动榜线看作时间序列来处理。程序会从历史活动中构造固定长度的序列窗口，输入每个时间点的分速、周末标记、日内周期特征、活动进度、剩余时间和活动强度等特征，训练 LSTM 去预测下一步的分速。

训练流程分两步：先使用全类型历史活动进行预训练，再在目标活动类型的数据上微调。损失函数同样偏向惩罚低估，并在最后 24 小时根据剩余时间对预测分速做动态放大，以尝试覆盖临近结束时的冲刺增长。

预测时，LSTM 会持续取最近 `WINDOW_SIZE` 个点组成输入序列，预测下一步分速，并把预测结果写回序列缓冲区继续递推。训练好的模型、特征和归一化器会缓存在 `models_lstm/` 中。
