...
import event_predictor_auto3 as core
...
@app.route('/qq_predict', methods=['GET'])
def get_qq_predict():
    """
    提供给 qqbot 的专用 API 接口
    读取本地的最新预测结果，并计算 current_pt, speed, end_time 然后返回。
    """
    event_id = core.get_latest_event_id()
    if not event_id:
        return jsonify({'success': False, 'error': 'No Event ID found'}), 404

    cache_file = get_latest_result_file_path(event_id)
    data = load_json(cache_file)
    if data and 'final_pt' in data:
        meta = core.get_event_metadata(event_id)
        
        current_pt = 0
        speed = 0
        if 'chart_data' in data and 'actual' in data['chart_data'] and len(data['chart_data']['actual']) > 0:
            actual = data['chart_data']['actual']
            current_pt = actual[-1]['y']
            if len(actual) > 1:
                curr_time = actual[-1]['x']
                for pt in reversed(actual):
                    time_diff = curr_time - pt['x']
                    # 寻找至少半小时前的数据来计算时速
                    if time_diff > 30 * 60 * 1000:
                        speed = int((current_pt - pt['y']) / (time_diff / 3600000))
                        break
                        
        resp_data = {
            'success': True,
            'event_id': event_id,
            'final_pt': data['final_pt'],
            'current_pt': current_pt,
            'speed': speed,
            'end_time': meta['end_ts'] if meta else 0,
            'updated_at': data.get('data_timestamp', data.get('updated_at', int(time.time() * 1000)))
        }
        if 'chart_data' in data:
            resp_data['chart_data'] = data['chart_data']
        return jsonify(resp_data)
    else:
        return jsonify({'success': False, 'error': 'No completed prediction found'}), 404
